...
class ConcreteObserverFake(Observador):
    def __init__(self, obj):
        self.ObservadorFake = obj
        self.ObservadorFake.Agregar(self)

    def Update(self, value):
        print("Actualización dentro de ConcreteObserverAlta")
        self.estado = self.ObservadorFake.GetEstado()
        print("Estado = ", self.estado)
        print("----------------------------------------------------")
        log_instante = str(datetime.datetime.now())
        log_mensaje = "Alta de Registro Creado Aleatoriamente"
        informe_log = (log_instante + " - " + log_mensaje)
        print(informe_log)
        print("----------------------------------------------------")
        log(informe_log)